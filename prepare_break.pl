...
sub usage
{
	print "perl $0 -m allele_count -b hmm_output -c chrsizes.txt -s sampleID -o output\n";
}
...
sub main
{
	...
	$input_file = new IO::File($hmm_file, "r") or die "could not open $hmm_file: $!\n";
	while(my $l = $input_file->getline)
	{
		if(substr($l,0,1)eq "#")
		{
			...
			for(my $i=0; $i<@basecaller;++$i)
			{
				...
				#print @internal_line."\n";
				#last;
				$file_writer->print($sampleID."\t".$chr."\t".$internal_line[1]."\t".$basecaller[$i]."\t".$hmm[$i]."\t".$hmm_masked[$i]."\t".$internal_line[2]."\t".$internal_line[3]."\t".$internal_line[4]."\t".$internal_line[5]."\n");					
				if ($background ne $hmm[$i] and $background ne "UN" and $background ne "start") {
					$file_writer2->print($sampleID."\t".$chr."\t".$last_start."\t".$last_pos."\t".$background."\n");
					$last_start = $internal_line[1];
				} 
				
				if ($background ne "UN") {
					$background = $hmm[$i];
					$last_pos = $internal_line[1];
				}
			}
		#	print($sampleID."\n");
		#	print($chr."\n");
		...
		#	print($chrsizes{$chr}."\n");
		#	print($background."\n");
			$file_writer2->print($sampleID."\t".$chr."\t".$last_start."\t".$chrsizes{$chr}."\t".$background."\n");
		}
	}	
}
